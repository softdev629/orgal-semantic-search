...
async def insert_embedding(product_id: int, description: str):
    ...
    if check_validity(product_id, description):
        ...
        execute_values(cur, "INSERT INTO embeddings (product_id, chunk, tokens, embedding) VALUES %s;", data_list)
        conn.commit()
...
async def match(product_id: int, chunk: str, keyword: str):
    response = await client.chat.completions.create(
        ...
        messages=[
            { "role": "system", "content": f"You are 0/1 classifier. If the following chunk doesn't match the search keyword '{keyword}' or has negative meaning from it, output 0. Otherwise, 1. Output 0 or 1 only. No descriptions.\n\n{chunk}" },
        ]
    )
...
async def enrich(keyword: str):
    response = await client.chat.completions.create(
        ...
        model="gpt-3.5-turbo-1106",
        messages=[
            { "role": "system", "content": f"""I'm running a E-commerce company.
...
async def get_products(keyword: str, count: str, pre_ids: list):
    ...
    cur.execute(f"SELECT product_id, chunk FROM embeddings ORDER BY embedding <=> %s LIMIT {count}", (str(embedding), ))
    products = cur.fetchall()
    ids = list(dict.fromkeys(await asyncio.gather(*[match(product[0], product[1], keyword) for product in products if product[0] not in pre_ids])))
    if 0 in ids:
        ids.remove(0)
    return ids
...
async def pre_search(keyword: str):
    ...
    cur = conn.cursor()
    # Get the top 30 most similar documents using the KNN <=> operator
    cur.execute(f"SELECT product_id, chunk FROM embeddings WHERE chunk LIKE %s LIMIT 100", ("%".join(keys), ))
    products = cur.fetchall()
    ids = list(dict.fromkeys(await asyncio.gather(*[match(product[0], product[1], keyword) for product in products])))
    if 0 in ids:
        ids.remove(0)
    return ids
...
async def search(keyword: str):
    ...
    products = await asyncio.gather(*[get_products(key, count, pre_ids) for key in keys])
...
async def search_by_voice(audiofile: UploadFile= File(...)):
    try:
        ...
        transcript = await client.audio.transcriptions.create(model="whisper-1", file=file)
